...
public:
	...
	void SymmetricEncrypt(RandomNumberGenerator &rng, const byte *key, const byte *plainText, size_t plainTextLength, byte *cipherText, const NameValuePairs &parameters) const
	{
		...
		memset(block, 0, modulusLen - 1);
		memcpy(block + modulusLen - 2 - plainTextLength, plainText, plainTextLength);
		block[modulusLen - 2] = (byte) plainTextLength;

		Integer block_int = Integer(block, modulusLen - 1); 
		block_int = block_int + 1;
		block_int = a_exp_b_mod_c(block_int, Integer(2), p);

		a_times_b_mod_c(Integer(key, modulusLen), block_int, p).Encode(cipherText, modulusLen);
	}
	...
	DecodingResult SymmetricDecrypt(const byte *key, const byte *cipherText, size_t cipherTextLength, byte *plainText, const NameValuePairs &parameters) const
	{
		...
		Integer q = (p - 1) / 2;
		Integer q_plus1_div2 = (q + 1) / 2; 

		m = a_exp_b_mod_c(m, q_plus1_div2, p);
		if(m > q){
			m = p - m;
		}
		m = m - 1;
